...
- Exceção: favoritos não têm backend — são persistidos no `localStorage` (`lib/favorites.ts`), portanto rodam em Client Components por necessidade
...
- Design tokens ficam em `app/globals.css` (Tailwind 4 é CSS-first: `@theme` + variáveis CSS, não há `tailwind.config.ts`)
